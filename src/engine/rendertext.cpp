...
static void text_color(char c, char *stack, int size, int &sp, bvec color, int a) 
{
	...
	else
    {
		...
		if (c == 'b')	//blinking
		{
			if (lastmillis%1000 < 350) a = 0;
			c = stack[sp-1];
		} //else if (c == 'l') c = ((lastmillis%800)/100)+'0';
		switch(c)
        {
   ...
   //         case '7': color = bvec(255, 255, 255); break;   // white
			//case '8': color = bvec(192,  96, 255); break; // violet
			...
			case 'l':	//colorloop
			{
				...
				if      (t < 100) color = bvec( 64, 255, 128);	// green: player talk
				else if (t < 200) color = bvec( 96, 160, 255);	// blue: "echo" command
