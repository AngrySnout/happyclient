...
const char *getchatconline(int line)
{
	int numl = conlines.length();
	for (int i = 0; i < numl; i++)
	{
		if (!(conlines[i].type&(1<<8)) && !(conlines[i].type&(1<<9))) continue;
		if (--line <= 0) return conlines[i].line;
	}
	return NULL;
}
...
int drawconlines(int conskip, int confade, int conwidth, int conheight, int conoff, int filter, int y = 0, int dir = 1, float heightmult = 1)
{
...
int renderconsole(int w, int h, int abovehud)                   // render buffer taking into account time & scrolling
{
    ...
    int y = drawconlines(conskip, fullconsole ? 0 : confade, fragconsole? conwidth*2/3: conwidth, conheight, conpad+conoff, fullconsole ? fullconfilter : confilter&(~CON_IRC));
    if(!fullconsole && (miniconsize && miniconwidth))
	{
		if (fragconsole) drawconlines(0, confade, conwidth/3, conheight, conpad+conoff+conwidth*2/3, fragconfilter, conpad+conoff, 1, 1.2);
