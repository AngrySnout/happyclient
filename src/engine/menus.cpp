...
void guiplayers(uint *header, int *pagemin)
{
    extern const char *showplayers(g3d_gui *cgui, uint *header, int pagemin);
    if(cgui) 
    {
        const char *command = showplayers(cgui, header, *pagemin);
        if(command)
        {
            updatelater.add().schedule(command);
            if(shouldclearmenu) clearlater = true;
...
void guidemos(uint *header, int *pagemin)
{
    extern const char *showdemos(g3d_gui *cgui, uint *header, int pagemin);
    if(cgui) 
    {
        const char *command = showdemos(cgui, header, *pagemin);
        if(command)
        {
            updatelater.add().schedule(command);
            if(shouldclearmenu) clearlater = true;
