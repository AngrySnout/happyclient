...
void renderbackground(const char *caption, Texture *mapshot, const char *mapname, const char *mapinfo, bool restore, bool force)
{
    ...
    loopi(restore ? 1 : 3)
    {
		if (backgroundmillis == -1) backgroundmillis = lastmillis;
        glColor3f(1, 1, 1);
		...
		// 0.0 - 0.3
		//float bu = w/1920.0f, bv = h/1080.0f;
		//float cu = 0;
		float cu = sqrt((float)min(5000, lastmillis-backgroundmillis)/5000.0f)/20;
        glBegin(GL_TRIANGLE_STRIP);
        //glTexCoord2f(0,  0);  glVertex2f(0, 0);
        //glTexCoord2f(bu, 0);  glVertex2f(w, 0);
        //glTexCoord2f(0,  bv); glVertex2f(0, h);
        //glTexCoord2f(bu, bv); glVertex2f(w, h);
        glTexCoord2f(cu,    cu);  glVertex2f(0, 0);
        glTexCoord2f(cu+bu, cu);  glVertex2f(w, 0);
        glTexCoord2f(cu,    cu+bv); glVertex2f(0, h);
        glTexCoord2f(cu+bu, cu+bv); glVertex2f(w, h);
        glEnd();
...
int main(int argc, char **argv)
{
	...
	if (fragconsole)
	{
		confilter = confilter&(~(1<<11));
		confilter = confilter&(~(1<<13));
	}
	// end temporary
