...
namespace game
{
    ...
    void killed(fpsent *d, fpsent *actor)
    {
        ...
        else if(isteam(d->team, actor->team))
        {
            ...
            if(actor==player1)
			{
				...
				if (doautosorry)
				{
					defformatstring(text)(autosorry, dname);
					if (autosayteam) sayteam(text);
					else toserver(text);
				}
			}
            else if(d==player1)
			{
				...
				if (doautonp)
				{
					defformatstring(text)(autonp, aname);
					if (autosayteam) sayteam(text);
					else toserver(text);
				}
			}
    ...
    void drawhudicons(fpsent *d)
    {
        ...
        if(d->state!=CS_DEAD)
        {
            ...
            draw_textf((d->ammo[d->gunselect]<=ammohighlight)? "\fl%d": "%d", (HICON_X + 2*HICON_STEP + HICON_SIZE + HICON_SPACE)/2, HICON_TEXTY/2, d->ammo[d->gunselect]);
        }
    ...
    {
        static const char *names[] =  { "ping ", "players ", "mode ", "map ", "time ", "master ", "host ", "port ", "description ", "country " };
        static const float struts[] = { 7,       7,          12.5f,   14,      7,      8,         14,      7,       24.5f,          5};
    ...
    bool serverinfoentry(g3d_gui *g, int i, const char *name, int port, const char *sdesc, const char *map, int ping, const vector<int> &attr, int np, const char* country)
    {
        if(ping < 0 || attr.empty() || attr[0]!=PROTOCOL_VERSION)
        {
            switch(i)
            {
				...
				case 9:
					defformatstring(cflag)("%s.png", country); //country_flags/
					if(g->buttonf("%s", 0xFFFFDD, cflag, country)&G3D_UP) return true;
					break;
