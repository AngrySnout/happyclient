...
namespace game
{
    ...
    void switchname(const char *name)
    {
        if(name[0])
        {
			...
			addplayerwhois(player1->clientnum);
        }
    ...
    const char *getclientip(int cn)
    {
        fpsent *d = getclient(cn);
        static char ip[20];
		formatstring(ip)("%d.%d.%d.*", d->ip%256, (d->ip/256)%256, (d->ip/(256*256))%256);
        return ip;
    }
    ICOMMAND(getclientip, "i", (int *cn), result(getclientip(*cn)));
