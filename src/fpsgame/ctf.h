...
#endif
{
	...
	fpsent *getflagowner(int i)
	{
		return flags[i].owner;
	}
    ...
    void drawblip(fpsent *d, float x, float y, float s, const vec &pos, bool flagblip)
    {
        ...
        float size = flagblip ? 0.1f : 0.05f,
              xoffset = (flagblip&&!flagblipnooff) ? -2*(3/32.0f)*size : -size,
    ...
    void takeflag(fpsent *d, int i, int version)
    {
        ...
        else
		{
			...
			if (isteam(d->team, player1->team)) setsvar("flagholder", teamcolorname(d));
			else setsvar("enemyflagholder", teamcolorname(d));
		}
