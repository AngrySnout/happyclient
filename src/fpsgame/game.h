...
struct fpsent : dynent, fpsstate
{
    ...
    vec lastcollect;
    int frags, flags, deaths, totaldamage, totalshots, ext_teamkills, ext_accuracy;
    editinfo *edit;
    ...
    fpsent() : weight(100), clientnum(-1), privilege(PRIV_NONE), lastupdate(0), plag(0), ping(0), lifesequence(0), respawned(-1), suicided(-1), lastpain(0), attacksound(-1), attackchan(-1), idlesound(-1), idlechan(-1), frags(0), flags(0), deaths(0), totaldamage(0), totalshots(0), ext_teamkills(0), ext_accuracy(0), edit(NULL), smoothmillis(-1), playermodel(-1), ai(NULL), ownernum(-1), muzzle(-1, -1, -1)
    {
...
namespace game
{
    struct clientmode
    {
		...
		virtual fpsent *getflagowner(int i) { return 0; }
